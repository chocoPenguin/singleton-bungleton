...
**중요 지시사항:**
1.  이전에 요청한 프롬프트의 영향을 받지 않도록 한다.
2.  각 사용자가 서로 다른 문제를 받을 수 있도록 다양하고 독창적인 {num_questions} * {user_count}개의 문제를 생성해주세요.
3.  같은 주제나 개념이라도 다른 관점이나 접근 방식으로 문제를 출제해주세요.
4.  각 문제의 배점('max_score')은 1인당 총점 100점을 준수하여 문항별 난이도를 고려해 알아서 설정해 주세요.
5.  Resource Context가 정의되어 있지 않을 경우 지식으로 연결된 SharePoint 내에 존재하는 문서 범위 내에서만 문제를 출제한다.
6.  Resource Context가 정의되어 있고 경로 언급이 있는데 해당 경로가 SharePoint 내에 존재하지 않거나 해당 경로 내 파일이 없을 경우 문제를 출제할 수 없다는 result를 json 형태로 리턴하도록 한다.
...
1-1. 문제 유형이 "M"일 경우,
1-1-1. 객관식 문항으로 정답이 정확히 일치할 경우에 max_score, 일치하지 않을 경우에 0점을 부여하세요.
1-1-2. 문항에 포함된 모든 선택지에 대해 각 설명을 feedback에 추가하세요.
1-2. 문제 유형이 "S"일 경우,
1-2-1. 문항의 정답에 꼭 들어가야하는 필수 keyword가 정답에 포함되었는지를 기준으로 평가하세요.
1-3. 문제 유형이 "L"일 경우,
1-3-1. 문항의 정답에 핵심 keyword를 3개를 뽑으십시오.
1-3-2. 해당 핵심 keyword가 1개 이상 포함되었는지를 기준으로 평가하세요.
1-3-3. 핵심 keyword가 포함되어 있지 않더라도, 문맥상 내용이 일치하는 경우 부분점수를 부여하세요.
1-3-4. 핵심 keyword들을 피드백에 설명하세요.
